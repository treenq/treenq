...
run-e2e-test-env:
	...
	COMPOSE_BAKE=true docker compose -p treenq-e2e -f docker-compose.e2e.yaml up kube -d
	sleep 1
	$(SED) 's#https://127.0.0.1:6443#https://kube:6443#g' k3s_data/k3s/k3s.yaml
	COMPOSE_BAKE=true docker compose -p treenq-e2e -f docker-compose.e2e.yaml up -d --build
	while [ -z '$$(docker ps -q --filter "name=treenq-e2e-server")' ]; do sleep 1; done
	docker cp k3s_data/k3s/k3s.yaml $$(docker ps -q --filter "name=treenq-e2e-server"):/app/kubeconfig.yaml
	COMPOSE_BAKE=true docker compose -p treenq-e2e -f docker-compose.e2e.yaml restart server
	@echo "Checking e2e test environment is running..."
	until $$(curl --output /dev/null --silent --fail http://localhost:18000/healthz); do printf '.'; sleep 1; done && echo "Service Ready!"
	echo 'Service has been started'
